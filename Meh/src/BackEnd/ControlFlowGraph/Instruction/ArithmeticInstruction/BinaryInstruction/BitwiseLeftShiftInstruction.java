...
public class BitwiseLeftShiftInstruction extends BinaryInstruction {
    public BitwiseLeftShiftInstruction(VirtualRegister destination, Operand operand1, Operand operand2) {
    ...
    public static Instruction getInstruction(Operand destination, Operand operand1, Operand operand2) {
        if (destination instanceof VirtualRegister) {
            return new BitwiseLeftShiftInstruction((VirtualRegister)destination, operand1, operand2).rebuild();
        }
    ...
    public String toString() {
        return String.format("%s = shl %s %s", destination, operand1, operand2);
    }
}
