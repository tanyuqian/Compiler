...
public class NotEqualToInstruction extends BinaryInstruction {
    public NotEqualToInstruction(VirtualRegister destination, Operand operand1, Operand operand2) {
    ...
    public static Instruction getInstruction(Operand destination, Operand operand1, Operand operand2) {
        if (destination instanceof VirtualRegister) {
            return new NotEqualToInstruction((VirtualRegister)destination, operand1, operand2).rebuild();
        }
    ...
    public String toString() {
        return String.format("%s = sne %s %s", destination, operand1, operand2);
    }
}
