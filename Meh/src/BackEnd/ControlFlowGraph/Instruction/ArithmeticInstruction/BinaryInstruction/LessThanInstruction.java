 ...
 */
public class LessThanInstruction extends BinaryInstruction {
    public LessThanInstruction(VirtualRegister destination, Operand operand1, Operand operand2) {
    ...
    public static Instruction getInstruction(Operand destination, Operand operand1, Operand operand2) {
        if (destination instanceof VirtualRegister) {
            return new LessThanInstruction((VirtualRegister)destination, operand1, operand2).rebuild();
        }
    ...
    public Instruction rebuild() {
        if (operand1 instanceof ImmediateValue && operand2 instanceof ImmediateValue) {
            ...
            return MoveInstruction.getInstruction(destination, new ImmediateValue(value1 < value2 ? 1 : 0));
        }
        return this;
    }
    @Override
    public String toString() {
        return String.format("%s = slt %s %s", destination, operand1, operand2);
    }
}
