...
public class AdditionInstruction extends BinaryInstruction {
    public AdditionInstruction(VirtualRegister destination, Operand operand1, Operand operand2) {
    ...
    public static Instruction getInstruction(Operand destination, Operand operand1, Operand operand2) {
        if (destination instanceof VirtualRegister) {
            return new AdditionInstruction((VirtualRegister)destination, operand1, operand2).rebuild();
        }
    ...
    public String toString() {
        return String.format("%s = add %s %s", destination, operand1, operand2);
    }
}
