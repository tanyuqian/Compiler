...
public class MultiplicationExpression extends BinaryExpression {
	private MultiplicationExpression(Type type, boolean isLeftValue, Expression left, Expression right) {
	...
	public static Expression getExpression(Expression left, Expression right) {
		if (left.type instanceof IntType && right.type instanceof IntType) {
			...
			return new MultiplicationExpression(IntType.getType(), false, left, right);
		}
		throw new CompilationError("two int-type expressions are expected in the multiplication expression");
	}
	...
	public void emit(List<Instruction> instructions) {
		...
		instructions.add(MultiplicationInstruction.getInstruction(operand, left.operand, right.operand));
	}
}
