...
import Compiler.BackEnd.ControlFlowGraph.Instruction.ArithmeticInstruction.BinaryInstruction.GreaterThanInstruction;
...
public class GreaterThanExpression extends BinaryExpression {
	private GreaterThanExpression(Type type, boolean isLeftValue, Expression left, Expression right) {
	...
	public static Expression getExpression(Expression left, Expression right) {
		if (left.type instanceof IntType && right.type instanceof IntType) {
			...
			return new GreaterThanExpression(BoolType.getType(), false, left, right);
		...
		throw new CompilationError("two int-type or string-type expressions are expected in the greater-than expression");
	}

	@Override
	public String toString() {
		return "[expression: greater-than]";
	}
	...
	public void emit(List<Instruction> instructions) {
		...
		instructions.add(GreaterThanInstruction.getInstruction(operand, left.operand, right.operand));
	}
}
