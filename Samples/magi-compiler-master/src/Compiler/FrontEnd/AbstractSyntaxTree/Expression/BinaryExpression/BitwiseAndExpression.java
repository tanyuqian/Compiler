...
import Compiler.BackEnd.ControlFlowGraph.Instruction.ArithmeticInstruction.BinaryInstruction.BitwiseAndInstruction;
...
public class BitwiseAndExpression extends BinaryExpression {
	private BitwiseAndExpression(Type type, boolean isLeftValue, Expression left, Expression right) {
	...
	public static Expression getExpression(Expression left, Expression right) {
		if (left.type instanceof IntType && right.type instanceof IntType) {
			...
			return new BitwiseAndExpression(IntType.getType(), false, left, right);
		}
		throw new CompilationError("two int-type expressions are expected in the bitwise-and expression");
	}

	@Override
	public String toString() {
		return "[expression: bitwise-and]";
	}
