...
public class BitwiseXorInstruction extends BinaryInstruction {
	private BitwiseXorInstruction(VirtualRegister target, Operand source1, Operand source2) {
	...
	public static Instruction getInstruction(Operand target, Operand source1, Operand source2) {
		if (target instanceof VirtualRegister) {
			return new BitwiseXorInstruction((VirtualRegister)target, source1, source2).rebuild();
		}
	...
	public String toString() {
		return String.format("%s = xor %s %s", destination, source1, source2);
	}
}
