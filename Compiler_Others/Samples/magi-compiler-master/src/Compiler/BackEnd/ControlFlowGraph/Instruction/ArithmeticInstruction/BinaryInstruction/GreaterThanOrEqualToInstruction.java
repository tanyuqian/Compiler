...
public class GreaterThanOrEqualToInstruction extends BinaryInstruction {
	private GreaterThanOrEqualToInstruction(VirtualRegister target, Operand source1, Operand source2) {
	...
	public static Instruction getInstruction(Operand target, Operand source1, Operand source2) {
		if (target instanceof VirtualRegister) {
			return new GreaterThanOrEqualToInstruction((VirtualRegister)target, source1, source2).rebuild();
		}
	...
	public String toString() {
		return String.format("%s = sge %s %s", destination, source1, source2);
	}
}
